...
mod tests {
    ...
    fn deck_new_test() {
        let deck = Deck::new();
        assert_eq!(deck.full.len(), 52, "Full deck length should be 52");
        assert_eq!(
            deck.current.len(),
            52,
            "Current deck length should start as 52"
        );
        assert_ne!(deck.full, deck.current, "Current deck should be shuffled");
    }
    ...
    fn deck_pop() {
        ...
        assert_eq!(
            deck.full.len(),
            52,
            "Full deck length should not be changed after popping"
        );
        assert_eq!(deck.current.len(), 0, "Current deck length should be 0");
